...
import { recoverOtpValidator } from '#validators/recover_otp_validator'
...
export default class SessionTotpRecoverController {
  ...
  async store({ auth, session, request, response, i18n }: HttpContext) {
    const { recoveryCode } = await request.validateUsing(recoverOtpValidator)
    const user = await getUserById(session.get(TOTP_USER_ID_KEY))
    ...
    if (!isValid) {
      throw new FormError(i18n.t('errors.recoveryCodeInvalid'))
    }
