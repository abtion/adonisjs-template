...
import { verifyValidator } from '#validators/totp_validator'
import FormError from '#exceptions/form_error'

export default class ProfileTotpController {
  async store({ auth, security, response, i18n }: HttpContext) {
    security.ensureConfirmed()

    const user = auth.user!
    if (user.totpEnabled) throw new FormError(i18n.t('errors.totpAlreadyEnabled'))
  ...
  async verify({ auth, request, response, i18n }: HttpContext) {
    ...
    if (!totpSecret || !totpRecoveryCodes) {
      throw new FormError(i18n.t('errors.totpSecretNotGenerated'))
    }

    const isValid = adonis2fa.verifyToken(totpSecret, otp, totpRecoveryCodes)
    if (!isValid) throw new FormError(i18n.t('errors.otpInvalid'))
  ...
  async destroy({ auth, response, security, i18n }: HttpContext) {
    const user = auth.user!
    if (!user.totpEnabled) throw new FormError(i18n.t('errors.userWithout2FAActive'))
  ...
  async regenerateRecoveryCodes({ auth, security, response, i18n }: HttpContext) {
    const user = auth.user!
